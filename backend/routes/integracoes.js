...
module.exports = function(pool, auth, registrarLog) {
                    ...
                    if (novoStatus === 'pago') {
                        ...
                        if (parseInt(pendentes2.rows[0].n) === 0) {
                            ...
                            try {
                                ...
                                if (cliRes.rowCount > 0) await pool.query("UPDATE clientes SET status_cobranca = 'quitado' WHERE id = $1", [cliRes.rows[0].cliente_id]);
                            } catch (e2) {}
                        }
                    }
                }
    ...
    router.post('/asaas/criar-cobranca', auth, async (req, res) => {
        try {
            ...
            if (!customerId) {
                ...
                if (novo.id) { customerId = novo.id; await pool.query('UPDATE clientes SET asaas_customer_id = $1 WHERE id = $2', [customerId, cliente_id]); }
                else return res.status(400).json({ success: false, error: 'Erro ao criar cliente no Asaas', details: novo });
            }
            ...
            if (cobranca.id) res.json({ success: true, asaas_id: cobranca.id, bankSlipUrl: cobranca.bankSlipUrl, invoiceUrl: cobranca.invoiceUrl, pixQrCode: cobranca.pixQrCodeUrl });
            else res.status(400).json({ success: false, error: 'Erro ao criar cobrança', details: cobranca });
        } catch (error) {
    ...
    router.post('/asaas/criar-parcelas/:acordoId', auth, async (req, res) => {
        try {
            ...
            if (!customerId) {
                ...
                if (novo.id) { customerId = novo.id; await pool.query('UPDATE clientes SET asaas_customer_id = $1 WHERE id = $2', [customerId, ac.cliente_id]); }
            }
    ...
    router.get('/sync/status', auth, async (req, res) => {
        try {
            ...
            try { const teste = await asaasRequest('/customers?limit=1'); asaasStatus = { connected: true, totalCustomers: teste.totalCount || 0 }; } catch (e) { asaasStatus = { connected: false, error: e.message }; }
            res.json({ success: true, data: { local: { clientes: parseInt(local.rows[0].clientes), cobrancas: parseInt(cob.rows[0].total), sincronizadas: parseInt(cob.rows[0].sincronizadas) }, asaas: asaasStatus, ambiente: ASAAS_ENV } });
        } catch (error) {
    ...
    router.post('/sync/completa', auth, async (req, res) => {
        try {
            ...
            let offset = 0;
            while (true) {
                const response = await asaasRequest(`/customers?offset=${offset}&limit=100`);
                ...
                for (const customer of response.data) {
                    ...
                    if (existe.rowCount > 0) { await pool.query('UPDATE clientes SET nome = COALESCE($1, nome), email = COALESCE($2, email), telefone = COALESCE($3, telefone), asaas_customer_id = $4, asaas_sync_at = NOW() WHERE id = $5', [customer.name, customer.email, customer.phone, customer.id, existe.rows[0].id]); resultados.clientes.atualizados++; }
                    else { await pool.query('INSERT INTO clientes (nome, email, telefone, cpf_cnpj, empresa_id, asaas_customer_id, asaas_sync_at, status, created_at) VALUES ($1, $2, $3, $4, $5, $6, NOW(), \'ativo\', NOW())', [customer.name, customer.email, customer.phone, customer.cpfCnpj, empresa_id, customer.id]); resultados.clientes.importados++; }
                }
            ...
            offset = 0;
            while (true) {
                const response = await asaasRequest(`/payments?offset=${offset}&limit=100`);
                if (!response.data || response.data.length === 0) break;
                for (const payment of response.data) {
                    try {
                        ...
                        if (existe.rowCount > 0) { await pool.query('UPDATE cobrancas SET status = $1, valor_pago = CASE WHEN $1 = \'pago\' THEN $2 ELSE valor_pago END, data_pagamento = CASE WHEN $1 = \'pago\' THEN $3 ELSE data_pagamento END, asaas_sync_at = NOW() WHERE id = $4', [statusInterno, payment.value, payment.paymentDate, existe.rows[0].id]); resultados.cobrancas.atualizados++; }
    ...
    router.get('/pdf/cobranca/:id', auth, async (req, res) => {
        try {
            ...
            doc.moveDown();
            if (c.credor_nome) { doc.fontSize(14).fillColor('#1e3a5f').text('DADOS DO CREDOR'); doc.fontSize(11).fillColor('#000').text(`Nome: ${c.credor_nome}`); doc.moveDown(); }
    ...
    router.get('/pdf/acordo/:id', auth, async (req, res) => {
        try {
            ...
            doc.moveDown();
            if (parcelas.rowCount > 0) { doc.fontSize(14).fillColor('#1e3a5f').text('PARCELAS'); doc.fontSize(10).fillColor('#000'); parcelas.rows.forEach(p => { doc.text(`${p.numero}. ${formatarData(p.data_vencimento)} - ${formatarMoeda(p.valor)} - ${p.status?.toUpperCase()}`); }); }
            doc.moveDown(2);
            doc.fontSize(10).fillColor('#000');
            doc.text('_________________________________', 50); doc.text('Devedor'); doc.moveDown(2);
            doc.text('_________________________________', 50); doc.text('Credor/Representante'); doc.moveDown(2);
            doc.fontSize(8).fillColor('#999').text(`Documento gerado em ${new Date().toLocaleString('pt-BR')}`, { align: 'center' });
            doc.end();
    ...
    // GET /api/integracoes/configuracoes (ou /api/configuracoes via alias no app.js)
    router.get('/configuracoes', auth, async (req, res) => {
        try {
            ...
            // Remover campos sensíveis - nunca expor ao frontend
            delete config.smtp_pass;
            delete config.asaas_api_key;
            delete config.openai_api_key;
            // suri_token: retorna mascarado se existir (só mostra que está configurado)
            if (config.suri_token) {
                config.suri_token_configurado = true;
                config.suri_token = ''; // não envia o valor real
            } else {
                config.suri_token_configurado = false;
            }
    ...
    // PUT /api/integracoes/configuracoes (ou /api/configuracoes via alias no app.js)
    router.put('/configuracoes', auth, async (req, res) => {
        try {
            ...
            const campos = [
                ...
                'email_remetente', 'asaas_api_key', 'asaas_ambiente',
                'nome_empresa', 'percentual_comissao_padrao',
                'dias_aviso_vencimento', 'openai_api_key', 'ia_ativa',
                'juros_atraso', 'multa_atraso',
                // ← campos suri agora salvos no banco
                'suri_token', 'suri_identificador', 'suri_endpoint'
            ];
            ...
            campos.forEach(campo => {
                // suri_token: só atualiza se vier um valor não vazio (não sobrescreve com vazio)
                if (campo === 'suri_token') {
                    if (b[campo] !== undefined && b[campo] !== '' && b[campo] !== null) {
                        updates.push(`${campo} = $${idx}`);
                        params.push(b[campo]);
                        idx++;
                    }
                } else if (b[campo] !== undefined) {
                    updates.push(`${campo} = $${idx}`);
                    params.push(b[campo]);
                    idx++;
                }
    ...
    router.get('/dashboard', auth, async (req, res) => {
        try {
            const cobrancas = await pool.query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = 'pendente') as pendentes, COUNT(*) FILTER (WHERE status = 'pago') as pagas, COUNT(*) FILTER (WHERE status = 'vencido' OR (status = 'pendente' AND data_vencimento < CURRENT_DATE)) as vencidas, COALESCE(SUM(valor_original), 0) as valor_total, COALESCE(SUM(valor_original) FILTER (WHERE status = 'pendente'), 0) as valor_pendente, COALESCE(SUM(valor_original) FILTER (WHERE status = 'pago'), 0) as valor_pago FROM cobrancas`);
